		...
		routes
			...
			.map((route) => {
				...
				if (Object.keys(urlParams).length) {
					...
					// ВАЖНО! Обязательно используем аккумулятор, так как нам нужно один и тот же объект несколько раз модифицировать
					return allParams.reduce((acc, param) => {
						return {
							...
							path: acc.path.replace(`:${param}`, urlParams[param]),
							name: acc.name,
						};
					}, route); // здесь route - начальное значение аккумулятора, то есть по сути acc = route
				}
